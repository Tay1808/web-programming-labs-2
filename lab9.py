...
def final():
    try:
        ...
        if age < 18:
            if gender == 'male':
                message = f"Поздравляю тебя, {name}! Желаю, ты был самым счатливым и добрым! Будь послушным и честным, С Новым годом!"
            else:
                message = f"Поздравляю тебя, {name}! Желаю, чтобы ты была самой счатливой и красивой! Будь послушной и заботливой, с Новым годом!"
        else:
            if gender == 'male':
                message = f"Поздравляю вас, {name}! Желаю успехов в работе, здоровья и радости! С Новым годом!"
            else:
                message = f"Поздравляю вас, {name}! Желаю счастья, красоты и вдохновения! С Новым годом!"

        # Логика выбора изображения
        if preference1 == 'something_tasty':
            if preference2 == 'sweet':
                image = 'slad.jpg'
                message += " Вот тебе подарок — сладости."
            else:
                image = 'eat.jpg'
                message += " Вот тебе подарок — праздничное ужин."
        else:
            if preference2 == 'refined':
                image = 'dog.jpg'
                message += " Вот тебе подарок — cамый лучший друг."
            else:
                image = 'NG.jpg'
